...
const resolvers = {
  Query: {
    ...
    me: async (_parent: any, _args: any, context: any) => {
      ...
      if (context.user) {
        return User.findOne({ _id: context.user._id }).populate('savedCharacters');
      }
  ...
  Mutation: {
    ...
    removeCharacter: async (_parent: any, { characterId }: RemoveCharacterArgs, context: any) => {
      if (context.user) {
      ...
      throw new AuthenticationError('You need to be logged in to remove a character!');
        },
    },
  };
